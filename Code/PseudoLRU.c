...
int UpdateLRU(unsigned int set, unsigned int line, int min, int max, int flag)
{
	...
	// If the request was made due to a cache hit, write LRU bits (no need to read)
	// ----------------------------------------------------------------------------
	if(flag == CACHE_HIT)
	{
	...
	// If the request was made due to a cache miss, find line to evict and write LRU bits
	// ----------------------------------------------------------------------------------
